...
* Details on the detailed Information needed to execute a workflow (i.e. which Specimen Detail Information is needed to execute a Laboratory Test, which Patient Detail Information is needed for a Referal, which Detail information needs to be present in a MedicationRequest or a ServiceRequest to be able to execute the Request). We describe ways how additional information should be transferred, but this details needs to be defined in more specific IGs). 
