...
* **Task** resources can serve many purposes. The default recommended mechanism to share or document a request for updating resources that are owned by another participant (e.g. a Fulfiller's request for cancelling an order) is to use a `Task` resource following the patterns described in this Implementation Guide. In this guide, a _Coordination Task_ serves a core role of helping a Placer and a _specific_ *potential*, or *eventual* Fulfiller manage the status of a request (in scenarios where FHIR servers are used). Many `Task`s MAY correspond to the same `ServiceRequest`, with a _Coordination Task_ for each Placer and (potential) Fulfiller pair. In environments where several potential Fulfillers may each contribute a partial output (such as multiple pharmacies each providing a partial dispense), Placers may also initiate a "parent" _Coordination Task_ that they own and only they may modify, in addition to the shared Fulfiller-specific _Coordination Tasks_. The overall status of the workflow may be represented using Request.status and the presence of Outputs or, when a parent Task exists, via the parent `Task.businessStatus` and `Task.status`. The page [using Task](requests-events-tasks.html) provides additional guidance on the use of the `Task` resource. For example, if a hospital would like to coordinate transportation assistance for a patient, they may create a single "transportation `ServiceRequest`" for that patient, with one `Task` per potential provider of transportation (each of which references the same `ServiceRequest`), or separate `ServiceRequest` and `Task` resources per Fulfiller. Placers and Fulfillers must pre-coordinate where their shared _Coordination Task_ will be hosted to ensure there is always an agreed upon source of truth. 
